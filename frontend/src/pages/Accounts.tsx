...
const Accounts: React.FC = () => {
  ...
  const handleInstagramLoginSubmit = async (credentials: { username: string; password: string }) => {
    try {
      setIsSubmittingInstagram(true);
      console.log('📷 [ACCOUNTS DEBUG] Submitting Instagram credentials');
      
      await apiClient.post('/api/platforms/instagram/login', {
        username: credentials.username,
        password: credentials.password,
        platform_username: credentials.username,
      });
      
      console.log('✅ [ACCOUNTS DEBUG] Instagram login successful');
      ...
      console.error('❌ [ACCOUNTS DEBUG] Instagram login error:', err);
      const errorData = err.response?.data?.error;
      const errorMessage = errorData?.message || errorData || 'Failed to connect Instagram';
      throw new Error(errorMessage);
    } finally {
